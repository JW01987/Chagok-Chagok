...
> ⚠️ 이 문서의 대부분은 초기 Node.js 버전(`scripts/auto-pilot.js`) 기준으로 쓰여 있습니다.
> 실제로 현재 쓰이는 구현은 Python(`scripts/auto_pilot.py`, `python scripts/auto_pilot.py`로 실행)입니다.
> 아래 "텔레그램에서 Claude Code와 대화하기" 섹션은 실제 Python 구현 기준으로 최신 상태입니다.
...
## 텔레그램에서 Claude Code와 대화하기

> 2026-07-28 추가.

`claude -p`(비대화형 실행)는 한 번 실행하면 끝까지 돌거나, 애매한 상황(예: 이미 완료된 작업을 또 만들라고 시켰을 때)에서는 작업을 끝내지 않고 질문으로 턴을 마칩니다. 예전에는 그 질문이 auto-pilot을 켜둔 터미널에만 찍히고 텔레그램으로는 전혀 오지 않아서, 진이가 그 자리에 없으면 그대로 멈춰 있었습니다.

지금은 다음과 같이 동작합니다:

1. `/시작`, `/다시시작` 처리 중 PASS 후 다음 태스크 실행, RETRY(다시 생성) 등 Claude Code를 실행하는 모든 경로에서 `claude -p ... --output-format json`으로 실행하고, 응답 텍스트를 세션 ID와 함께 파싱합니다.
2. Claude의 응답은 항상 "🤖 Claude Code 응답" 메시지로 텔레그램에 옵니다 — 작업을 끝냈든, 질문을 했든 상관없이.
3. 그 세션 ID를 `.auto-pilot-state.json`의 `last_claude_session`에 저장해둡니다.
4. 대기 중인 세션이 있는 상태에서 `/시작`, `/다시시작`, `/취소`가 아닌 일반 텍스트를 텔레그램으로 보내면, 그 텍스트를 `claude -p --resume <session_id> "<답장>"`으로 그대로 전달해서 같은 세션을 이어갑니다.
5. `/취소`를 보내면 대기 중인 세션을 종료합니다 (이후 일반 텍스트는 다시 안내 메시지로 처리됨).

즉, "이대로 진행할까요, 다른 작업으로 넘어갈까요?" 같은 질문이 오면 터미널을 열어보지 않고 텔레그램에서 그냥 답장하면 됩니다.

**주의**: 세션은 명시적으로 `/시작`으로 새 태스크를 시작하거나 `/취소`를 보내기 전까지 계속 "대기 중"으로 남아있습니다. 이미 끝난 작업에 대한 응답 뒤에 관련 없는 텍스트를 보내면 그 세션에 그대로 전달되니, 헷갈리면 `/취소`부터 보내는 걸 추천합니다.
